...
def paired_percentile_bootstrap(differences: Sequence[float], samples: int, seed: int) -> tuple[float, float] | None:
    if not differences:
        return None
...
def ranking_metrics(top_ids: Sequence[str], direct_ids: Iterable[str], k: int) -> dict[str, float | int]:
...
def bootstrap_from_protocol(protocol: object, differences: Sequence[float]) -> tuple[float, float] | None:
    config = protocol.statistics["paired_percentile_bootstrap"]
    if config["samples"] != 10000 or config["seed"] != 20260809 or config["ci_type"] != "PERCENTILE":
        raise ValueError("bootstrap configuration diverges from frozen protocol")
    return paired_percentile_bootstrap(differences, config["samples"], config["seed"])
