"""
run_ablation_study.py — Orchestratore dello studio di ablazione per RQ1.

Esegue le 5 condizioni (vanilla, websearch, rag_testuale, enricher_only, full_graphrag)
...
# Import dei moduli di ablazione
from backend.evaluation.ablation_websearch import run_websearch, evaluate_websearch_pmids
...
# File dei risultati delle condizioni storiche
ZEROSHOT_HISTORICAL_JSON = RESULTS_DIR / "zeroshot_results.json"
GRAPHRAG_HISTORICAL_JSON = RESULTS_DIR / "benchmark_results.json"
...
def main():
    parser = argparse.ArgumentParser(description="Ablation study per RQ1")
    parser.add_argument("--case", type=str, default=None, help="Esegui solo un caso specifico (es. BENCH-004)")
    parser.add_argument("--run-all", action="store_true", help="Forza il ricalcolo completo anche di vanilla e full_graphrag")
    ...
    if args.case:
        ...
        print(f"Esecuzione dello studio di ablazione sul singolo caso: {args.case}")
    else:
        cases_to_run = benchmark_rows
        print(f"Esecuzione dello studio di ablazione su tutti i {len(cases_to_run)} casi")

    # 2. Carica i risultati storici se non è abilitato --run-all
    historical_vanilla = {}
    historical_graphrag = {}

    if not args.run_all and ZEROSHOT_HISTORICAL_JSON.exists():
        try:
            with open(ZEROSHOT_HISTORICAL_JSON, "r", encoding="utf-8") as f:
                historical_vanilla = {r["case_id"]: r for r in json.load(f)}
            print(f"Caricati {len(historical_vanilla)} casi Vanilla storici.")
        except Exception as e:
            print(f"Impossibile caricare Vanilla storico: {e}")

    if not args.run_all and GRAPHRAG_HISTORICAL_JSON.exists():
        try:
            with open(GRAPHRAG_HISTORICAL_JSON, "r", encoding="utf-8") as f:
                historical_graphrag = {r["case_id"]: r for r in json.load(f)}
            print(f"Caricati {len(historical_graphrag)} casi GraphRAG storici.")
        except Exception as e:
            print(f"Impossibile caricare GraphRAG storico: {e}")

    # Condizioni
    conditions = ["vanilla", "websearch", "rag_testuale", "enricher_only", "full_graphrag"]
    ...
    for cond in conditions:
        filename = filename_map.get(cond, f"ablation_{cond}_results.json")
        ...
        if out_file.exists() and not args.fresh and not args.case:
            try:
                ...
                # Ricalcola al volo le metriche se mancano in qualche record (es. rigenerati con phase0)
                for r in loaded_data:
                    if "dac_drug_found" not in r:
                        case_id = r["case_id"]
                        meta = benchmark_rows[case_id]
                        expected_drug = meta["expected_drug"]
                        expected_pmid = int(meta["pmid"]) if meta.get("pmid") and str(meta.get("pmid")).isdigit() else None
                        expected_escat = meta.get("escat", "")
                        explicit_tier = r.get("extracted_escat")
                        
                        metrics = evaluate_report(case_id, r["report"], expected_drug, expected_pmid, expected_escat, explicit_tier=explicit_tier)
                        r.update(metrics)
                        
                condition_results[cond] = loaded_data
    ...
    for case_id, row in cases_to_run.items():
        ...
        print(f"\n{'-'*40}\nElaborazione Caso {case_id}: {gene} {variant} ({tumor_type})\n{'-'*40}")

        # ── 0. VANILLA ──
        if not any(r["case_id"] == case_id for r in condition_results["vanilla"]):
            print("  -> Esecuzione Condizione: VANILLA...")
            if case_id in historical_vanilla and not args.run_all:
                print("     [INFO] Riuso il report Vanilla storico.")
                report = historical_vanilla[case_id]["report"]
            else:
            ...
            with open(RESULTS_DIR / "ablation_vanilla_results.json", "w", encoding="utf-8") as f:
                json.dump(condition_results["vanilla"], f, indent=2, ensure_ascii=False)

        # ── 1. WEBSEARCH ──
        if not any(r["case_id"] == case_id for r in condition_results["websearch"]):
            print("  -> Esecuzione Condizione: WEBSEARCH...")
            report = retry_on_exception(run_websearch, gene, variant, tumor_type, alteration_type, therapy_line)
            metrics = evaluate_report(case_id, report, expected_drug, expected_pmid, expected_escat)
            
            # Calcola anche le metriche PubMed separate per la web search
            web_pmid_eval = retry_on_exception(evaluate_websearch_pmids, report, {"gene": gene, "variant": variant, "tumor_type": tumor_type})
            
            judge = retry_on_exception(llm_as_judge, report, {"gene": gene, "variant": variant, "tumor_type": tumor_type})
            
            condition_results["websearch"].append({
                "case_id": case_id,
                "report": report,
                **metrics,
                "pmid_exists_rate": web_pmid_eval["pmid_exists_rate"],
                "pmid_relevant_rate": web_pmid_eval["pmid_relevant_rate"],
                "web_pmid_detail": web_pmid_eval,
                "judge_score": judge.get("score_totale"),
                "judge_detail": judge
            })
            with open(RESULTS_DIR / "ablation_websearch_results.json", "w", encoding="utf-8") as f:
                json.dump(condition_results["websearch"], f, indent=2, ensure_ascii=False)

        # ── 2. RAG TESTUALE ──
        if not any(r["case_id"] == case_id for r in condition_results["rag_testuale"]):
            print("  -> Esecuzione Condizione: RAG TESTUALE...")
            report = retry_on_exception(run_rag_testuale, gene, variant, tumor_type, alteration_type, therapy_line)
            metrics = evaluate_report(case_id, report, expected_drug, expected_pmid, expected_escat)
            judge = retry_on_exception(llm_as_judge, report, {"gene": gene, "variant": variant, "tumor_type": tumor_type})
            
            condition_results["rag_testuale"].append({
                "case_id": case_id,
                "report": report,
                **metrics,
                "judge_score": judge.get("score_totale"),
            ...
            with open(RESULTS_DIR / "ablation_rag_results.json", "w", encoding="utf-8") as f:
                json.dump(condition_results["rag_testuale"], f, indent=2, ensure_ascii=False)

        # ── 3. ENRICHER ONLY ──
        if not any(r["case_id"] == case_id for r in condition_results["enricher_only"]):
            print("  -> Esecuzione Condizione: ENRICHER ONLY...")
            report = retry_on_exception(run_enricher_only, gene, variant, tumor_type, alteration_type, therapy_line)
            metrics = evaluate_report(case_id, report, expected_drug, expected_pmid, expected_escat)
            judge = retry_on_exception(llm_as_judge, report, {"gene": gene, "variant": variant, "tumor_type": tumor_type})
            
            condition_results["enricher_only"].append({
                "case_id": case_id,
                "report": report,
                **metrics,
                "judge_score": judge.get("score_totale"),
            ...
            with open(RESULTS_DIR / "ablation_enricher_results.json", "w", encoding="utf-8") as f:
                json.dump(condition_results["enricher_only"], f, indent=2, ensure_ascii=False)

        # ── 4. FULL GRAPHRAG ──
        if not any(r["case_id"] == case_id for r in condition_results["full_graphrag"]):
            print("  -> Esecuzione Condizione: FULL GRAPHRAG...")
            if case_id in historical_graphrag and not args.run_all:
                print("     [INFO] Riuso il report GraphRAG storico.")
                report = historical_graphrag[case_id]["report"]
                explicit_tier = historical_graphrag[case_id].get("escat_tier", "")
            else:
            ...
            # Se riusiamo lo storico, riusiamo anche il punteggio judge se presente
            if case_id in historical_graphrag and not args.run_all and historical_graphrag[case_id].get("judge_score") is not None:
                judge = historical_graphrag[case_id]["judge_detail"]
            else:
                judge = retry_on_exception(llm_as_judge, report, {"gene": gene, "variant": variant, "tumor_type": tumor_type})
            ...
            with open(RESULTS_DIR / "ablation_graphrag_results.json", "w", encoding="utf-8") as f:
    ...
    for cond in conditions:
        ...
        judge_compl = [r.get("judge_detail", {}).get("completezza") for r in results if r.get("judge_detail") and r.get("judge_detail", {}).get("completezza") is not None]
        judge_util = [r.get("judge_detail", {}).get("utilita_clinica") for r in results if r.get("judge_detail") and r.get("judge_detail", {}).get("utilita_clinica") is not None]
        judge_fed = [r.get("judge_detail", {}).get("fedelta_evidenze") for r in results if r.get("judge_detail") and r.get("judge_detail", {}).get("fedelta_evidenze") is not None]
        judge_acc = [r.get("judge_detail", {}).get("accuratezza_clinica") for r in results if r.get("judge_detail") and r.get("judge_detail", {}).get("accuratezza_clinica") is not None]
        ...
        # Aggiungi le metriche extra per la web search
        if cond == "websearch":
            exists_rates = [r.get("pmid_exists_rate", 0.0) for r in results]
            relevant_rates = [r.get("pmid_relevant_rate", 0.0) for r in results]
            summary_row["pmid_exists_pubmed"] = round(sum(exists_rates) / len(exists_rates), 3) if exists_rates else 0.0
            summary_row["pmid_relevant_pubmed"] = round(sum(relevant_rates) / len(relevant_rates), 3) if relevant_rates else 0.0
        else:
            summary_row["pmid_exists_pubmed"] = "N/A"
            summary_row["pmid_relevant_pubmed"] = "N/A"
            
        summary_rows.append(summary_row)

    # 4. Stampa la tabella comparativa markdown in console
    print("\n### TABELLA RIASSUNTIVA COMPARATIVA (STUDIO DI ABLAZIONE)")
    print(
        f"{'Condizione':<16} | {'DAC':>6} | {'PMID Cov':>8} | {'PMID Hall':>9} | {'ESCAT':>6} | "
        f"{'Judge Tot':>9} | {'Compl':>5} | {'Util':>5} | {'Fed':>5} | {'Acc':>5} | {'PMID Ex.':>8} | {'PMID Rel.':>9}"
    )
    print("-" * 115)
    for r in summary_rows:
        ...
        print(
            ...
            f"{fmt_f(r['judge_score_totale']):>9} | {fmt_f(r['judge_completezza']):>5} | "
            f"{fmt_f(r['judge_utilita_clinica']):>5} | {fmt_f(r['judge_fedelta_evidenze']):>5} | "
            f"{fmt_f(r['judge_accuratezza_clinica']):>5} | "
            f"{fmt_p(r['pmid_exists_pubmed']):>8} | {fmt_p(r['pmid_relevant_pubmed']):>9}"
        )

    # 5. Salva la tabella in CSV
    csv_out = RESULTS_DIR / "ablation_summary.csv"
    with open(csv_out, "w", encoding="utf-8", newline="") as f:
        fieldnames = [
            "condizione", "drug_anchor_check", "pmid_coverage_rate", "pmid_hallucination_rate",
            "escat_tier_match", "judge_score_totale", "judge_completezza", "judge_utilita_clinica",
            "judge_fedelta_evidenze", "judge_accuratezza_clinica", "pmid_exists_pubmed", "pmid_relevant_pubmed"
        ]
