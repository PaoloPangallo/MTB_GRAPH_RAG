"""La rigenerazione degli artefatti di adjudication, contro il gold.
...
Stava in `backend/tests/test_multi_intervention_adjudication.py`, dove si dichiarava saltato quando il bundle
...
from backend.tests import test_multi_intervention_adjudication as CORE
...
class RebuildTests(unittest.TestCase):
    ...
    def test_rebuilding_reproduces_the_committed_artifacts(self) -> None:
        for name, content in CORE.build().items():
            with self.subTest(artifact=name):
                self.assertEqual(
                    content, (CORE.ADJ / name).read_text(encoding="utf-8")
                )
