...
| C4 | La catena GCA → documento → SourceUnit → quote è ricostruibile end-to-end | RQ2 grounding + LIVE | `INDEPENDENT` + `HELD_OUT` | E5, E7 · RQ2 | `document_resolution_rate`, `parser_success_rate`, catena di eventi completa | `rq2/`, `live_replay/` | la catena è ricostruibile su ogni run completata | la copertura è limitata dalla disponibilità documentale, non dall'architettura |
...
| C12 | Il layer di presentazione non può alterare il dossier | narrative held-out + controlli | `HELD_OUT` | E6 · Ablation D | `failed_narrative_presented = 0 / 20`, specificità `5 / 5` | `ablations/narrative_verifier.json` | sensibilità sui 20 ostili **e** specificità sui 5 controlli | senza i controlli positivi la sensibilità non sarebbe interpretabile |
...
| C16 | LIVE non richiede bundle congelati | LIVE | — | E7 | `frozen_bundle_access = 0`, `bundle_source_unit_ids_used = false` | `live_replay/unseen_document.json` | 0 su tutte le run LIVE | — |
| C17 | REPLAY è riproducibile e senza rete | REPLAY | `DEVELOPMENT_REGRESSION` | E8 | `network_calls = 0 / 25`, `live_selector_calls = 0 / 25`, accordo sugli hash | `live_replay/replay_reproducibility.json` | 0 e 0, output canonico identico | riproducibilità, mai performance |
...
| equivalenza LIVE = REPLAY | ruoli operativi diversi; nessuna metrica di accuratezza li confronta |
