...
heldout_bundle_sha256 : ecf6183a2eef8068c7f0292f561f980ed758bcce224b279eee3ffd67adc57467
...
| CASE_ID | INPUT_SUMMARY | EXPECTED_PATH | GOLD_RATIONALE | OVERLAP | NOTES |
|---|---|---|---|---|---|
| `HO-AMB-01-abbreviation-collision` | Patient with MCC. CDKN2A loss on the panel. Considering palbociclib. | eligibility ∈ {AMBIGUOUS_CASE_CONTEXT, MISSING_REQUIRED_FIELDS, INSUFFICIENT_ONCOLOGY_CONTEXT}; retrieval=no; stop=stage_3b_pre_retrieval_eligibility_gate; run=CONTROLLED_STOP | 'MCC' è usato sia per Merkel cell carcinoma sia, in molti referti, per metastatic colorectal cancer. Le due letture portano a candidate diverse e il testo non contiene nulla che disambigui: CDKN2A e… | — | nessuna candidate ancorata: la proprietà valutata è il percorso, non il contenuto documentale — resolution_class=STOP_OR_REVIEW |
...
| `HO-AMB-04-two-readings-of-question` | Endometrial carcinoma, CCNE1 amplification. Camonsertib. Alternatives? | eligibility ∈ {AMBIGUOUS_CASE_CONTEXT, MISSING_REQUIRED_FIELDS, INSUFFICIENT_ONCOLOGY_CONTEXT}; retrieval=no; stop=stage_3b_pre_retrieval_eligibility_gate; run=CONTROLLED_STOP | Non è deducibile se il farmaco vada valutato, se sia già stato usato, o se se ne chiedano di diversi. Le tre letture portano a retrieval diversi. | — | nessuna candidate ancorata: la proprietà valutata è il percorso, non il contenuto documentale — resolution_class=STOP_OR_REVIEW |
...
| CASE_ID | INPUT_SUMMARY | EXPECTED_PATH | GOLD_RATIONALE | OVERLAP | NOTES |
|---|---|---|---|---|---|
| `HO-CON-01-two-primary-diseases` | Patient with high-grade serous ovarian carcinoma. The primary tumour is a uterine corpus endometrial carcinoma. CCNE1 amplification detected. Is camo… | eligibility ∈ {CONTRADICTORY_CASE_CONTEXT}; retrieval=no; stop=stage_3b_pre_retrieval_eligibility_gate; run=CONTROLLED_STOP | Due primitivi mutuamente esclusivi. Sceglierne uno cambierebbe il candidato recuperato; il sistema non ha l'autorità per farlo. | — | nessuna candidate ancorata: la proprietà valutata è il percorso, non il contenuto documentale |
...
| `HO-CON-04-question-premise-conflict` | Gastrointestinal stromal tumour. No molecular testing has been performed and no alteration is known. Explain the resistance mechanism of the KIT T670… | eligibility ∈ {CONTRADICTORY_CASE_CONTEXT, MISSING_REQUIRED_FIELDS}; retrieval=no; stop=stage_3b_pre_retrieval_eligibility_gate; run=CONTROLLED_STOP | La domanda presuppone un'alterazione che il testo dichiara assente. La premessa della domanda contraddice i fatti del caso. | — | nessuna candidate ancorata: la proprietà valutata è il percorso, non il contenuto documentale |
...
| `HO-INC-02-missing-biomarker` | Patient with metastatic uveal melanoma of the choroid. The oncology team is evaluating tebentafusp. Molecular profiling has been requested but no res… | eligibility ∈ {MISSING_REQUIRED_FIELDS, INSUFFICIENT_ONCOLOGY_CONTEXT}; retrieval=no; stop=stage_3b_pre_retrieval_eligibility_gate; run=CONTROLLED_STOP | L'assenza del biomarker è dichiarata nel testo, non implicita: il sistema non deve né inventarlo né procedere senza. | A4-contradicted-or-resistance (1×5-gram), CASE-4-contradicted-or-resistance (1×5-gram) | nessuna candidate ancorata: la proprietà valutata è il percorso, non il contenuto documentale |
...
| `NH-POL-03` | BD-01 (AMBIGUOUS) | polarity_inversion | direction UNCERTAIN -> negative | VERIFIER_SHOULD_REJECT | afferma che la letteratura esclude il farmaco, invertendo un'incertezza in negazione |
...
| near-duplicate 5-grammi · boilerplate | 5 |
...
| `HO-INC-02-missing-biomarker` | CASECONTEXT_ROBUSTNESS_35 | `A4-contradicted-or-resistance` | the oncology team is evaluating | boilerplate |
| `HO-INC-02-missing-biomarker` | END_TO_END_PIPELINE_PILOT_5 | `CASE-4-contradicted-or-resistance` | the oncology team is evaluating | boilerplate |
...
Da compilare dal revisore umano prima del freeze. Finché resta vuoto,
`frozen` rimane `false` e la final evaluation non parte.
...
| casi rifiutati | |
| gold contestati | |
| esito | ACCEPTED / REVISION_REQUIRED |
